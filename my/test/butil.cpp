...
void invokeInp(int h, int i, int (*readSense)(int port)){
  int reads = inp[h].numreadings;
  srand(time(0));
  for (int k=0;k<reads;k++){
    int nval = readSense(inp[h].port[0]);//(max-min +1) + min
    ...
    if(nval!=oval){
      ...
      for(int m=0;m<inp[h].tar[k].numtargs;m++){
        ...
        /* for relays with tsec>0*/
        int tsec;
        if(i == 1 & oval == 0 & nval ==1 & srs[sr].data[1]>0){//i=switch
          tsec =srs[sr].data[1];
