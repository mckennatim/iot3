...
Edgewater operation can be kicked off by pressing a button. It just turns on the first relay. In edgewater, there is only one target, sr = 0, and the data to change is data[0], onoff. `inp.i_updInputs()` changes the state of an sr relay
...
* else if srs[sr].priosr which is srs[priosr].onoff=off and srs[sr].prionoff has beeen prg set to 1
