...
* for `toggle`, if a button is pressed, the target(s) of that button get changed. The first get is the sr, `int sr = inp[ix].tar[0].gets[m][0];` ,<s> the second `gets[m][1]` is 0 for off and 1 for on </s> but that doesn't matter cause it is a toggle, it just toggles the sr's data value `srs[sr].data[da] = !srs[sr].data[da];` and sets a flag that it has changed
