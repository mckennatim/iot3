...
* it finds an on relay, or if it finds an off relay whose prioronoff=1 is tores tsec in 
* if it finds a relay set to on, it stores tsec and flips the realy if different
* if it finds the relay set to off with a prionoff=1 
