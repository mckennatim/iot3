...
* Checks all the on relays `(f.ISrELAYoN & sr2) ==sr2 `
  ...
  * double checks if relay is on. If someone re-tapped the button it could be off.
  * When `inp.i_updCtrl()` finds a relay set to on, it saves tmsec in `sTRTsWtIMR[sr]`. If the elapsed time is greater than the setting (.data[1]*1000), then the relay gets turned off, f.ISrELAYoN gets unset
