...
* Checks all the on relays `(f.ISrELAYoN & sr2) ==sr2 `
  ...
  * When `inp.i_updCtrl()` finds a relay set to on, it saves tmsec in `sTRTsWtIMR[sr]`. If the elapsed time is greater than the setting (.data[1]*1000), then the relay gets turned off, 
