...
.then((addrinfo)=>{
  ...
  fs.mkdir(devdir, {recursive:true}, (err)=>{
    ...
    if(!fs.existsSync(`${projdir}/appdata.js`)){
      ...
      appdat.write(`const cfgdata=${JSON.stringify(cfgdata,null,2)}\n\n`)
      appdat.write(`const senstypes=${JSON.stringify(senstypes,null,2)}\n\n`)
      appdat.write(`const ctrltypes=${JSON.stringify(ctrltypes,null,2)}\n\n`)
      appdat.write(`const topics=${JSON.stringify(topics,null,2)}\n\n`)
      appdat.write(exp)
      appdat.end()
    }
  })
