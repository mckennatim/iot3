...
void doSt(int sr, const int *FLAG){
  printf("pow2(%d)= %d, flag is %d \n", sr, pow2(sr), *FLAG  );
}
...
void i_updInputs(){
  for(int ix=0;ix<NUMINP;ix++){
    for(int sen=0;sen<SENSTYPS;sen++){
      if(strcmp(sensors[sen],inp[ix].itype)==0){
        switch (sen){
          case 0: //butn
...
void setXdata(char* idev, int sr, int darr[]){
  ...
  int idx = getXdataIdx(didx ,sr);
  if(idx>-1){
    for (size_t i = 0; i < xdata[idx].nd; i++){
      int data = darr[xdata[idx].dt[i].xdidx];
      for (size_t j = 0; j < xdata[idx].dt[i].targs.numtargs; j++){
        int tsr = xdata[idx].dt[i].targs.gets[j][0];
        int tda = xdata[idx].dt[i].targs.gets[j][1];
        srs[tsr].data[tda] = data;
      }
    }
  }
}
